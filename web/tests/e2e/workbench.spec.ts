...
test("核心工作中台浏览器流：在线接单人、DDL、日程、网盘留言和 TTS 友好错误", async ({ browser }) => {
  ...
  try {
    ...
    await page.getByPlaceholder(/必须基于项目证据/).fill(`${projectName} 的网盘里有什么？`);
    await page.getByRole("button", { name: /让 Agent grep/ }).click();
    await expect(page.getByText(/证据|没有找到可靠依据/).first()).toBeVisible({ timeout: 20000 });
