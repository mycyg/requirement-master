...
async function saveScreenshot(page: Page, name: string) {
  ...
  const finalPath = path.join(screenshotDir, name);
  const tmpPath = path.join(screenshotDir, `${name}.${Date.now()}.tmp.png`);
...
async function keepWorkerOnline(browser: Browser, nickname: string) {
  ...
  await expect(page.getByText(/实时连接|实时在线/)).toBeVisible();
...
test("核心工作中台浏览器流：在线接单人、DDL、日程、网盘留言和 TTS 友好错误", async ({ browser }) => {
  ...
  try {
    ...
    let projectRow = page.locator("li").filter({ hasText: projectName }).first();
    await projectRow.getByRole("button", { name: "归档", exact: true }).click();
    await expect(page.getByRole("heading", { name: "归档项目" })).toBeVisible();
    await page.getByLabel("输入项目名确认").fill(projectName);
    await page.getByRole("button", { name: "确认归档" }).click();
    await page.getByRole("button", { name: "已归档" }).click();
    await expect(page.getByText(projectName, { exact: true })).toBeVisible();
    projectRow = page.locator("li").filter({ hasText: projectName }).first();
    await projectRow.getByRole("button", { name: "恢复", exact: true }).click();
    await page.getByLabel("输入项目名确认").fill(projectName);
    ...
    await page.getByRole("link", { name: /提\s*一(个|条)新?需求/ }).click();
    ...
    const descBox = page.getByPlaceholder(/团队周报模板/);
    await expect(descBox).toHaveValue(/语音输入 E2E/);
    await descBox.fill(`${requirementText}\n${await descBox.inputValue()}`);
    ...
    await expect(page.getByRole("button", { name: /保存并继续|下一步/ })).toBeDisabled();
    await page.getByLabel(/截止时间/).fill(localDateTime(24 * 60));
    ...
    await expect(page.getByRole("heading", { name: new RegExp(`${projectName}|项目网盘`) })).toBeVisible();
    ...
    await page.getByRole("button", { name: "e2e-note.md", exact: true }).click();
    await expect(page.getByText(/E2E Drive/)).toBeVisible();
    await saveScreenshot(page, "04_drive_preview.png");
    ...
    await expect(page.getByRole("heading", { name: "E2E 会议纪要" })).toBeVisible();
    ...
    await page.goto(`/knowledge?project_id=${projectId}`);
    await expect(page.getByRole("heading", { name: /知识库|历史搜索/ })).toBeVisible();
    await page.getByPlaceholder(/搜需求编号/).fill("E2E Drive");
    await page.getByRole("button", { name: /grep|搜索/ }).click();
    await expect(page.getByText(/e2e-note|E2E Drive/).first()).toBeVisible({ timeout: 20000 });
    await page.getByPlaceholder(/基于项目历史/).fill(`${projectName} 的网盘里有什么？`);
    await page.getByRole("button", { name: /让 (Agent grep|AI 助理找证据)/ }).click();
    await expect(page.getByText(/证据|没有找到可靠依据|找不到证据/).first()).toBeVisible({ timeout: 20000 });
    await saveScreenshot(page, "07_knowledge.png");

    await page.goto(`/planning?project_id=${projectId}`);
    await expect(page.getByRole("heading", { name: "排期 / 负载" })).toBeVisible();
    await expect(page.getByText(workerNick).first()).toBeVisible();
    await saveScreenshot(page, "08_planning.png");

    await page.goto(`/notifications`);
    await expect(page.getByRole("heading", { name: "通知中心" })).toBeVisible();
    await saveScreenshot(page, "09_notifications.png");

    await page.goto(`/health?project_id=${projectId}`);
    await expect(page.getByRole("heading", { name: "项目健康度" })).toBeVisible();
    await expect(page.getByRole("heading", { name: projectName })).toBeVisible();
    await saveScreenshot(page, "10_health.png");
...
test("需求个人工作区 UI：进度、阻塞、清单和动态", async ({ browser }) => {
  ...
  await expect(page.getByRole("button", { name: /保存进度|已保存/ })).toBeVisible();
