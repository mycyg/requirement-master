...
├── scripts/                   # 部署/运维脚本（paramiko 操作 192.168.5.224）
...
首次启动弹窗：
- 服务端地址：默认 `http://192.168.5.224:8080`；旧 `192.168.0.x` 会自动迁到同尾号 `192.168.5.x`
- 昵称：你自己
- 同步根目录：默认 `D:\工作需求`
...
### 提需求方
1. 浏览器开 `http://192.168.5.224:8080`（或你的 `192.168.5.x` 服务地址）
2. 填昵称 → cookie 持久化
