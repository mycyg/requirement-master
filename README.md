...
| "测试项目建多了，列表像垃圾堆" | 项目支持归档、软删除和恢复；归档/删除都要二次确认，误操作先别急着写遗书 |
...
- 🧹 **项目归档 / 软删除**：项目列表支持正常、已归档、回收站三种视图；归档和删除都要输入项目名二次确认，删除默认保留数据可恢复
...
- 🔔 **本地托盘 / 工作台**：Python pystray 常驻，SSE 长连接 + 系统通知，菜单显示服务在线、接单状态、同步状态、最后心跳；pywebview 打开 `/local-workbench`，文件同步到本地目录
...
安装脚本会创建桌面快捷方式和开机启动项。托盘菜单第一项 "打开本地工作台"（默认双击）会启动 pywebview 并打开 `/local-workbench`，自动带昵称 cookie 和本地端 token；"打开 Web 派活端" 会进普通浏览器管理端；"打开项目保存位置" 会直接打开本地需求文件根目录。项目网盘同步默认关闭，可切到单向下载或双向同步；本地删除默认不会删远端，手滑党暂时安全。
...
Windows 装完后桌面会多一个 **需求管理大师 本地工作台** 快捷方式，启动项里会多一个 **需求管理大师**。如果右下角看不到图标，先点系统托盘隐藏区；它不是跑路了，只是 Windows 喜欢把正经人藏起来。
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts\smoke_client_install.ps1
```

这条会在临时目录模拟 Windows 安装，检查 `launch.ps1`、配置文件、桌面快捷方式、启动项快捷方式是否都生成；默认跳过 pip 下载，专门测安装脚本逻辑。
